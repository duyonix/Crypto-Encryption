...
# CBC
$openssl enc  -aes-128-cbc  -e -in test.txt -out cbc_cipher.bin \
-K 00112233445566778889aabbccddeeff \
-iv 0102030405060708
...
$echo -n "ABCDEFGHIJKLMNOP" > f3.txt # 16 bytes

# f1.txt encrypted with CBC -> p1.bin
$openssl enc  -aes-128-cbc  -e -in f1.txt -out p1.bin \
-K 00112233445566778889aabbccddeeff \
-iv 0102030405060708
$ls -ld p1.bin # 16

# f2.txt encrypted with CBC -> p2.bin
$openssl enc  -aes-128-cbc  -e -in f2.txt -out p2.bin \
-K 00112233445566778889aabbccddeeff \
-iv 0102030405060708
$ls -ld p2.bin # 16

# f3.txt encrypted with CBC -> p3.bin
$openssl enc  -aes-128-cbc  -e -in f3.txt -out p3.bin \
-K 00112233445566778889aabbccddeeff \
-iv 0102030405060708
$ls -ld p3.bin # 32
...
$openssl enc  -aes-128-cbc  -d -in p1.bin -out p1_decrypt.txt \
-K 00112233445566778889aabbccddeeff \
...
$openssl enc  -aes-128-cbc  -d -in p2.bin -out p2_decrypt.txt \
-K 00112233445566778889aabbccddeeff \
-iv 0102030405060708 -nopad
...
$openssl enc  -aes-128-cbc  -d -in p3.bin -out p3_decrypt.txt \
-K 00112233445566778889aabbccddeeff \
-iv 0102030405060708 -nopad
